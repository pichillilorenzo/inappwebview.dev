---
title: "Content Worlds"
sidebar_position: 4
date: 2022-10-12 12:00:00
description: Content worlds help prevent issues that occur when two scripts modify environment variables in conflicting ways.
---

import config from '../../../docusaurus.config';

<head>
  <title>Javascript Content Worlds | {config.title}</title>
</head>
...
InAppWebView(
  ...
  onLoadStop: (controller, url) async {
    ...
    var source = "${(!kIsWeb && defaultTargetPlatform == TargetPlatform.android)
        ? "window.top.document.body.innerHTML"
        : "document.body.innerHTML"} = 'Modifying the body HTML content from another Content World';";
